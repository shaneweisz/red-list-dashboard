...
import { TAXA, getTaxonConfig, CATEGORY_COLORS, CATEGORY_NAMES } from "@/config/taxa";
  ...
  try {
    if (taxonId === "all") {
      ...
      for (const subTaxon of subTaxa) {
        ...
        const gbifCsvPath = path.join(process.cwd(), "data", subTaxon.gbifDataFile);
        if (!fs.existsSync(gbifCsvPath)) continue;

        const redListNames = new Set(
          subData.species.map((s) => s.scientific_name?.toLowerCase?.() || "").filter(Boolean)
        );

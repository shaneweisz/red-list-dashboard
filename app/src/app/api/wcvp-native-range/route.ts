 ...
 * already carried on every species as `s.countries`. Also returns `powoId`, the
 * accepted taxon's own POWO/IPNI id, for linking out to its real POWO page
 * (https://powo.science.kew.org/taxon/urn:lsid:ipni.org:names:{powoId}).
 *
 * Queries the ~21MB `wcvp-native-countries.parquet` (bundled with this function,
...
export async function GET(request: NextRequest) {
  ...
  const result = await conn.runAndReadAll(
    `SELECT countries, powo_id FROM read_parquet($path) WHERE name = $name`,
    { path: PARQUET_PATH, name }
  );
  // DuckDB LIST columns come back from getRowObjects() as { items: [...] }, not a plain array.
  const rows = result.getRowObjects() as unknown as { countries: { items: string[] }; powo_id: string | null }[];
  const countries = rows.length > 0 ? rows[0].countries.items : null;
  const powoId = rows.length > 0 ? rows[0].powo_id : null;
  return NextResponse.json({ countries, powoId }, { headers: CACHE_5M });
}

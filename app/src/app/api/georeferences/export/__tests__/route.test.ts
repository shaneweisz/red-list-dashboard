/**
 * The export gate. CSV leaving the dashboard is the step that needs sign-in, so
 * the check lives in the route rather than only in the button's disabled state —
...
function signedIn(email = "assessor@example.org") {
...
describe("POST /api/georeferences/export", () => {
  ...
  it("returns a Darwin Core CSV attachment for a signed-in user", async () => {
    signedIn();
  ...
  it("checks auth before looking at the payload", async () => {
    signedOut();

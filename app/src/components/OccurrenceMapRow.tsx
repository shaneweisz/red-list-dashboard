...
import { QualityFlag, QUALITY_FLAG_LABELS } from "@/lib/coordinate-cleaning";
...
export default function OccurrenceMapRow({
  ...
  const flagDefs = useMemo(
    () =>
      (Object.keys(QUALITY_FLAG_LABELS) as QualityFlag[])
        .map((key) => ({ key, label: QUALITY_FLAG_LABELS[key], count: flagCounts[key] ?? 0 }))
        .filter((d) => d.count > 0),
    [flagCounts]
  );

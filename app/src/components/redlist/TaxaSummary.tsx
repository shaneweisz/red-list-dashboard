...
const numericThClasses = `${cellPad} text-right text-[13px] font-semibold text-zinc-500 dark:text-zinc-300 uppercase tracking-wider whitespace-nowrap w-0`;
const flexTdClasses = `${cellPad} whitespace-nowrap w-0`;
const flexThClasses = `${cellPad} text-left text-[13px] font-semibold text-zinc-500 dark:text-zinc-300 uppercase tracking-wider whitespace-nowrap w-0`;
...
export default function TaxaSummary({ onToggleTaxon, selectedTaxa }: Props) {
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-zinc-50 dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700">
              <th className={`${stickyClasses} bg-zinc-50 dark:bg-zinc-800 ${cellPad} text-left text-[13px] font-semibold text-zinc-500 dark:text-zinc-300 uppercase tracking-wider whitespace-nowrap w-0`}>Taxon</th>
  ...
  const renderBar = (percent: number, barColor: string, isAll: boolean) => {
    ...
    return (
      <div className="flex items-center gap-2 min-w-[160px] md:min-w-[220px]">
        ...
        <span className="text-base md:text-lg font-medium tabular-nums text-zinc-900 dark:text-zinc-100 w-[52px] text-right">
          {percent.toFixed(1)}%
        </span>
      </div>
    );
  ...
  const renderRow = (
    ...
    return (
      ...
      >
        ...
        {isVisible("described") && (
          <td className={numericTdClasses}>
            <span className="text-base md:text-lg font-medium text-zinc-900 dark:text-zinc-100 tabular-nums">
  ...
  const renderHead = () => (
    <thead>
      <tr className="bg-zinc-50 dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700">
        <th className={`${stickyClasses} bg-zinc-50 dark:bg-zinc-800 ${cellPad} text-left text-[13px] font-semibold text-zinc-500 dark:text-zinc-300 uppercase tracking-wider whitespace-nowrap w-0`}>
          <div className="flex items-center gap-1.5">
            Taxon
            <button
              ref={menuButtonRef}

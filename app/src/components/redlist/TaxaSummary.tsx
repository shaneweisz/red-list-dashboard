...
export default function TaxaSummary({ onToggleTaxon, selectedTaxa, selectedSubgroups, onToggleSubgroup, onNavigateToSubgroup, disableAllSpecies, viewMode = "reassessments" }: Props) {
  ...
  return (
    ...
    {/* Controls below the table: # Described source toggle (left, under that column)
        and the expand / Table 1a options (right, landing page only). */}
    {!loading && perTaxa.length > 0 && (selectedTaxa.size === 0 || isVisible("described")) && (
      <div className="flex items-center justify-between gap-3 mt-1.5">
        {/* IUCN ↔ CoL source toggle for "# Described Species", sitting directly below
            that column instead of in the header. Flips the count + recomputes % Assessed. */}
        {isVisible("described") ? (

 ...
 *   Phase 11b: build-name-index     (→ name-index.parquet, search)
...
import { run as buildNameIndex } from "./build-name-index";
...
async function main() {
  ...
  try {
    ...
    if (!taxaFilter) {
      ...
      // The other half of search's read layer: the name-sorted index over the fast
      // path (assessed ∪ GBIF-observed), so a typeahead prefix prunes to a row group
      // instead of scanning both species parquets. Needs species_link (phase 10) for
      // the col_ids it bakes in, so it can't move up next to build-parquet.
      console.log("\nPhase 11b: build-name-index (→ name-index.parquet, search)");
      console.log("═".repeat(60));

...
async function run(write: boolean): Promise<void> {
  ...
  console.log(`\n${unresolved} unresolved, ${overlapping} overlapping`);
  ...
  if (unresolved > 0 || overlapping > 0) {
    console.log("\nBoth conditions are failures, not warnings: an unresolved name is a group");
    console.log("missing species, and an overlap double-counts them.");
    process.exitCode = 1;
  }
}

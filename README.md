...
11b. `build-name-index` — every name the search fast path can match (scientific name, common name, epithet, and each word of a multi-word common name) → `name-index.parquet`, sorted by name so a typeahead prefix prunes to a row group instead of scanning `assessed`/`unassessed` whole. Needs `species_link.parquet` (phase 10) for the `col_id`s it bakes in, which is why it runs here rather than beside `build-parquet`

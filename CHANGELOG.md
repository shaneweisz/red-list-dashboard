...
- Reworked the species detail panel's Red List tab, now called **IUCN Red List Assessments**. Every narrative section (Rationale, Population, Habitat & Ecology, ...) is expanded on open — the expand/collapse toggles are gone, in the single-assessment view and the comparison view alike — and the Habitats and Threats code lists spell their qualifiers out on screen instead of hiding them in a hover tooltip: Suitability / Season / Major importance per habitat, Timing / Scope / Severity / Impact per threat, with the values that matter (Suitable, Major importance: Yes, Ongoing, Very Rapid Declines) picked out in colour. Classification codes now read as `7.3` rather than the API's `7_3`

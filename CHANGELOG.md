...
- Fixed the Suggested Assessors and Suggested Reviewers tabs coming up empty for any drilled-in taxon. Live taxonomic drilldown made a selection like Mammals → Rodentia → Muridae an id the static taxonomy tree has no node for, and the candidate query treated the whole id as a taxon-group name, so it matched nothing at all. A drilled-in selection is now suggested from its nearest static ancestor — Muridae's candidates come from Mammals — and the table says which group the ranking is over, so the counts aren't read as being about the narrower one

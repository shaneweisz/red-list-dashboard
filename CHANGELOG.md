...
- Added a scope option to the **Taxonomic differences from Catalogue of Life** chart, defaulting to **10+ yrs old only**. A taxonomic difference is worth acting on where the assessment is due for renewal — on a recent one the assessors already had the current checklist in front of them — so the reason bars, the ⚑/Clean totals and the ⚑ flags in the species table now cover only assessments last done more than 10 years ago (the same set the Needs Updating filter selects), and picking flagged/clean or a reason filters to those species too, so a bar's count is exactly what clicking it returns. "Any age" in the card's dropdown restores the old behaviour and brings back the flags on recently-assessed species; it travels in shared links as `colScope=all`

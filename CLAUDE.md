...
**Write it as one clear description of the current feature, not a phase-by-phase log.** A PR that lands in many rounds of feedback doesn't need a "Phase 1 / Phase 2 / ... Phase 15" section per round — that's what the commit history and review comments are for. Describe what the feature does and how it works *today*, as if writing it fresh; fold each round's changes into the relevant existing section instead of appending a new dated one. If the journey itself has real value (a design dead-end worth flagging so it isn't re-tried), a short "Bugs/wrong-turns caught along the way" section covers that without narrating every round.

...
| `cc_coun` | Point outside the record's reported country | Natural Earth country polygons | **Ported** (phase 5) — 236 countries, Natural Earth 1:50m admin-0 (patched for a France/Norway ISO_A2 data quirk, see §4); no buffer, matching upstream's own default. Complements rather than replaces GBIF's own `COUNTRY_COORDINATE_MISMATCH`, which doesn't catch every mismatch |
...
- **Country borders** (phase 5, shipped): Natural Earth's 1:50m admin-0 countries layer, same martynafford mirror — 236 countries (1,612 polygon parts after splitting MultiPolygon countries), rounded to 3 decimal places (~110m precision) like the land mask. Reconsidered from the original "lower priority, GBIF already covers most of this" verdict — cheap to add given the sea/urban point-in-polygon infrastructure already existed, and it catches mismatches GBIF's own `COUNTRY_COORDINATE_MISMATCH` doesn't. Required patching a real Natural Earth data quirk (France and Norway both carry a placeholder `ISO_A2 = "-99"` instead of their real codes) — see `app/src/lib/coordinate-cleaning-refdata/README.md` for details and the live-data regression check that confirms the fix.
